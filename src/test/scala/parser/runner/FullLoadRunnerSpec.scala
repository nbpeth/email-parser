...
class FullLoadRunnerSpec extends WordSpecLike with Matchers {
  ...
  private var mockFileProcessor: FileProcessor = _

  "process load" should {
    "call the field extractor for every file in batch and send them to the writer" in {
      setupMocks()
      val load = FullLoad.apply(mockFiles(10))
      val runner = new FullLoadRunner(mockWriter, mockFileProcessor, 10)

      runner.process(load)

      verify(mockFileProcessor, times(load.initialLoad)).apply(mockFile)
    ...
    "process nothing and proceed gracefully if the initial full load is empty" in {
      ...
      verify(mockFileProcessor, never()).apply(mockFile)
    ...
    "call the field extractor for each file and submit sub-batches to writer with a large and uneven file size" in {
      ...
      val runner = new FullLoadRunner(mockWriter, mockFileProcessor, 200)

      runner.process(batch)

      verify(mockFileProcessor, times(batch.initialLoad)).apply(mockFile)
  ...
  "nextFileFrom(batch)" should {
    "return Future of EmailMessage data when a file is passed in" in {
      ...
      val runner = FullLoadRunner(mockWriter, mockFileProcessor)
  ...
  def setupMocks(): Unit = {
    ...
    mockFileProcessor = mock(classOf[FileProcessor])

    when(mockFileProcessor.apply(mockFile)).thenReturn(mockEmailMessageData)
  }
